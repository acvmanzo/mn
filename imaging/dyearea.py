...
def loadarea(fname):
    ...
    arearatio = areadata[0]/areadata[1]
    return(arearatio)

def loadarea_3d2(fname):
    areadata = np.loadtxt(fname, skiprows=1, usecols=(1,))
    
    arearatio = math.pow(areadata[0], 1.5)/math.pow(areadata[1], 1.5)
    return(arearatio)


def writearea(fname, summpath, summnotes):
...
def writearea_3d2(fname, summpath, summnotes):
    
    area = loadarea_3d2(fname)
